...
include 'C:/xampp/htdocs/ss/model/Blog.php';
...
class blogC
{
    ...
    public function fav($idu, $idb)
    {
        $db = config::getConnexion();

        try {
            $checkSql = "SELECT COUNT(*) FROM fav WHERE idu = :idu AND idb = :idb";
            $checkQuery = $db->prepare($checkSql);
            $checkQuery->execute(['idu' => $idu, 'idb' => $idb]);
            $exists = $checkQuery->fetchColumn();
            ...
            if ($exists) {
                // Supprimer le like
                $deleteSql = "DELETE FROM fav WHERE idu = :idu AND idb = :idb";
                $deleteQuery = $db->prepare($deleteSql);
                $deleteQuery->execute(['idu' => $idu, 'idb' => $idb]);

                // JS Alert
                echo "<script>alert('supprimé de la liste favorie!'); window.location.href = 'index.php';</script>";
            } else {
                // Ajouter le like
                $insertSql = "INSERT INTO fav (idu, idb) VALUES (:idu, :idb)";
                $insertQuery = $db->prepare($insertSql);
                $insertQuery->execute(['idu' => $idu, 'idb' => $idb]);

                echo "<script>alert('ajouté a la liste favorie!'); window.location.href = 'index.php';</script>";
            }
        } catch (Exception $e) {
            echo "<script>alert('Erreur: " . addslashes($e->getMessage()) . "'); window.location.href = 'index.php';</script>";
        }
